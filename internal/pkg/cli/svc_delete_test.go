...
func TestDeleteSvcOpts_Execute(t *testing.T) {
	...
	tests := map[string]struct {
		...
		// A servie can be deployed to multiple
		...
		"happy path with environment passed in as flag": {
			...
			setupMocks: func(mocks deleteSvcMocks) {
				gomock.InOrder(
					...
					mocks.ecr.EXPECT().ClearRepository(mockRepo).Return(nil).Times(0),

					// It should **not** removeSvcFromApp
					mocks.appCFN.EXPECT().RemoveServiceFromApp(mockApp, mockSvcName).Return(nil).Times(0),

					// It should **not** deleteSSMParam
					mocks.store.EXPECT().DeleteService(mockAppName, mockSvcName).Return(nil).Times(0),
				)
			},
			wantedError: nil,
		},
